...
/**
 * Parses a `header` line in a report file or summary file.
 *
...
function splitHeader (s) {
  var header = { version: +s[1], date   : +s[2]
               , slot   :  s[3], rev    :  s[4]
  ...
  var match = header.slot.match(/-(n2\.[0-9])$/)
  if (match) header.branch = match[1].replace('n', 'v')

  return header
}

/**
 * Parses a `config` line in a report file or summary file.
 *
 * @param  {string} line Line to be splitted
 * @return {object|null} The resulting object, or null if an error occurred.
 *
...
/**
 * Parses a `stats` line in a summary file.
 *
 * @param  {string} line Line to be splitted
 * @return {object|null} The resulting object, or null if an error occurred.
 *
...
module.exports.Summary = function (h, c, s) {
  ...
  // Alias subarch to arch if not supplied
  if (!this.subarch && this.arch) this.subarch = this.arch
}
...
function loadSummary (slot, date, callback) {
  return new Promise(function (fulfill, reject) {
    var header, config, stats
    var error
    var repdir = join(util.dir, slot, date)
    ...
    fs.createReadStream(
      ...
      err.status = 404
      reject(err)
    }).pipe(csv({ delimiter: ':' })).on('data', function (line) {
      if (error) return
      function checkLine (obj, name) {
        if (!obj) {
          var msg = 'Could not parse ' + name + '.\n'
          error = new Error(msg)
          reject(error)
        }
      }
      ...
      if (error) return
      fulfill(new Summary(header, config, stats))
    }).on('error', function (err) {
      // Let's just try to skip parsing errors for now
    })
  })
}
...
module.exports.loadReport = function loadReport (slot, date, errdiff, cb) {
  return new Promise(function (fulfill, reject) {
    var repFile = join(slot, date, 'report.xz')
      , repPath = join(util.dir, repFile)
    ...
    var fileStream = fs.createReadStream(repPath)
      ...
      error = true
      err.status = 404
      err.message = 'Report "' + repFile + '" not found.'
      reject(err)
    })

    var recs = []

    fileStream.pipe(xz.d()).on('error', function (err) {
      if (error) return
      // xz-pipe is buggy so it emits two error events. Skip the first.
      if (err.errno) return
      error = true
      err.status = 500
      err.message = 'Report "' + repFile + '" is damaged.'
      reject(err)
    }).pipe(csv({ delimiter: ':' })).on('data', function handleLine (line) {
      if (error) return
...
module.exports.getSlotOwner = function (slot, callback) {
  return fs.readFileAsync(join(util.dir, slot, 'owner') , 'utf8')
  .call('trim')
  // ignore error
...
module.exports.loadLastPass = function (slot, callback) {
  return new Promise(function (fulfill, reject) {
    var repdir = join(util.dir, slot)
    var recs = {}
    var error

    var lastPass = fs.createReadStream(
      join(repdir, 'lastpass')
    ).on('error', function noSlotLastpass (err) {
      error = true
      err.message = 'Slot "' + slot + '" does not exist.'
      err.status = 404
      reject(err)
    }).pipe(csv({ delimiter: ':' })).on('data', function (splittedLine) {
      if (error) return
